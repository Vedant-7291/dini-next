...
export default function NewOrder() {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://dini-next.onrender.com/api/orders?status=pending');
      if (response.ok) {
  ...
  const handleRejectOrder = async (orderId) => {
    try {
      const response = await fetch(`https://dini-next.onrender.com/api/orders/${orderId}`, {
        method: 'DELETE',
      });
