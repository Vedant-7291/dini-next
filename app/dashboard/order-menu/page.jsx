...
export default function OrderMenu() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const acceptedResponse = await fetch('https://dini-next.onrender.com/api/orders?status=accepted');
      const acceptedData = acceptedResponse.ok ? await acceptedResponse.json() : [];
      
      // Fetch completed orders
      const completedResponse = await fetch('https://dini-next.onrender.com/api/orders?status=completed');
      const completedData = completedResponse.ok ? await completedResponse.json() : [];
      
      // Fetch stats
      const statsResponse = await fetch('https://dini-next.onrender.com/api/orders/stats');
  ...
  const markAsCompleted = async (orderId) => {
    try {
      const response = await fetch(`https://dini-next.onrender.com/api/orders/${orderId}/status`, {
        method: 'PATCH',
        headers: {
